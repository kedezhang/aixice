...
class CHttpRequest extends CApplicationComponent
{
	...
	{
	    if(isset($_GET[$name])){
	        $_GET[$name]=$this->inject_str_check($_GET[$name]);//sql注入过滤
	        $this->SafeFilter($_GET[$name]);
	        return $_GET[$name];
	    }else if(isset($_POST[$name])){
	        $_POST[$name]=$this->inject_str_check($_POST[$name]);//sql注入过滤
	        $this->SafeFilter($_POST[$name]);
	        return $_POST[$name];
	    }else{
	        return $defaultValue;
	    }
	...
	//将敏感字符串转化为全角 
	public function inject_str_check($str){
	    if($str&&!is_array($str)){
	        $arr=array(
	            'select'=>'ｓelect',
	            'insert'=>'ｉnsert',
	...
	function SafeFilter (&$arr)
	{
	   ...
	   /*  $ra=Array('/([\x00-\x08,\x0b-\x0c,\x0e-\x19])/','/script/','/javascript/','/vbscript/','/expression/','/applet/','/meta/','/xml/','/blink/','/link/','/style/','/embed/','/object/','/frame/','/layer/','/title/','/bgsound/','/base/','/onload/','/onunload/','/onchange/','/onsubmit/','/onreset/','/onselect/','/onblur/','/onfocus/','/onabort/','/onkeydown/','/onkeypress/','/onkeyup/','/onclick/','/ondblclick/','/onmousedown/','/onmousemove/','/onmouseout/','/onmouseover/','/onmouseup/','/onunload/');
	    ...
	    if (is_array($arr))
	    {
	        ...
	        $value= preg_replace($ra,'',$value);     //删除非打印字符，粗暴式过滤xss可疑字符串
	        $arr= htmlentities(strip_tags($value)); //去除 HTML 和 PHP 标记并转换为 HTML 实体
	    } */
	}
	/**
