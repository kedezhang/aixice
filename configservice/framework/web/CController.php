...
class CController extends CBaseController
{
    ...
    public function render($view,$data=null,$return=false)
    {
        if($this->beforeRender($view)) {
            //20201-1-11 wangjuns add

            if(!isset($data['ciId'])||!$data['ciId']){
                $ciId = Yii::app()->request->getParam('ciId');
                if ($ciId) {
                    $data['ciId'] = $ciId;
                }else{
                    $data['ciId']=null;
    ...
    protected function beforeRender($view)
    {
        ...
        if(!isset($session)||empty($session)){
            Yii::app()->clientScript->registerMetaTag(Yii::app()->request->fwqCsrfTokenName,Yii::app()->request->getFwqCsrfToken());
        }else{
            Yii::app()->clientScript->registerMetaTag(Yii::app()->request->_csrfToken,Yii::app()->request->getCsrfToken());
        }*/
        return true;
    }
