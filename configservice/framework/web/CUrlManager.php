...
class CUrlManager extends CApplicationComponent
{
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		//20210126修改
		$return_url=$this->createUrlDefault($route,$params,$ampersand).$anchor;
		if(strpos($return_url, 'ciId')===false){
		    $session_platId=isset($_SESSION['platIden'])?$_SESSION['platIden']:'';
		    if(!empty($session_platId)){
		        $return_url=$this->add_querystring_var($return_url, 'PlatID', $session_platId);
		    }else{
		        $ciId=isset($_REQUEST['ciId'])?$_REQUEST['ciId']:'';
		        if(!isset($ciId)||empty($ciId)){
		            $ciId="594806863904A9D8";
		        }
		        $return_url=$this->add_querystring_var($return_url, 'ciId', $ciId);
		    }
		}
		return $return_url;
	}
	function add_querystring_var($url, $key, $value) {
	    ...
	    if(strpos($url,'?') === false){
	        ...
	        if(strpos($url, '&')!==false){
	            ...
	            array_splice($url_list,1,0,$key.'='.$value);
